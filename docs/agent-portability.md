...
| Antigravity | Tools + skills | `~/.gemini/config/mcp_config.json` (or `agy mcp add`) + `~/.gemini/config/skills.json` for global skills | `README.md` Antigravity section, `~/.gemini/config/mcp_config.json`, `~/.gemini/config/skills.json` |
...
- **No Gemini CLI extension.** Antigravity (Google Antigravity CLI `agy` & IDE) runs Goldfish natively via stdio MCP (`mcp_config.json`) and Bun, so no extension packaging is needed.
...
- **One hooks map, two harnesses** (`hooks/goldfish-hooks.json`): Codex aliases `CLAUDE_PLUGIN_ROOT` to `PLUGIN_ROOT`, and the shared script emits the nested `hookSpecificOutput` SessionStart envelope with the guidance in `additionalContext` — so it needs no harness detection. Cursor keeps its separate native hook and payload.
