...
**Version 8.0.0** -- Fail-closed explicit workspace identity: user-level MCP registrations bind checkpoint, brief, and current-project recall calls to the conversation's host-native absolute project root; ambiguous candidates never authorize access. See CHANGELOG.md for details.
